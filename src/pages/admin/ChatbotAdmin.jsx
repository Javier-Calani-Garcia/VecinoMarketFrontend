...
export default function ChatbotAdmin() {
  ...
  function cargarEmpresas() {
    setCargando(true);
    return API.get('comunicacion/admin/resumen-empresas-chatbot/', { params: { q: busqueda || undefined } })
      .then((res) => {
  ...
  useEffect(() => {
    ...
    API.get('comunicacion/admin/interacciones-chatbot/', { params: { empresa: empresaSel.id } })
      .then((res) => setInteracciones(res.data))
      .catch(() => setError('No se pudo cargar las interacciones.'));
  }, [empresaSel]);

  if (cargandoAuth) return null;
  if (!usuario) return <Navigate to="/login?next=/admin/chatbot" replace />;
  if (!esStaff(usuario)) return <Navigate to="/" replace />;

  if (empresaSel) {
    return (
      <div className="mx-auto max-w-3xl px-4 py-8">
        <button onClick={() => { setEmpresaSel(null); cargarEmpresas(); }} className="flex items-center gap-1.5 text-sm text-gray-500 dark:text-gray-400 hover:text-brand-600 dark:hover:text-brand-400 mb-4">
        ...
        <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">CU15 · Historial de preguntas y respuestas de su chatbot.</p>

        {interacciones.length === 0 ? (
          <p className="text-sm text-gray-400 dark:text-gray-500 py-8 text-center">Sin interacciones todavía.</p>
        ) : (
          <div className="space-y-3">
            {interacciones.map((i) => (
              <div key={i.id} className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 p-4">
                <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{i.pregunta}</p>
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{i.respuesta}</p>
                <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">{new Date(i.fecha).toLocaleString()}</p>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8">
      <div className="flex items-center gap-2 mb-1">
        <Bot className="text-brand-600 dark:text-brand-400" size={24} />
        <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Chatbot de atención</h1>
      </div>
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">CU15 · Cuántas FAQ configuró cada empresa y cuánto se usa su chatbot.</p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {empresas.map((e) => (
            ...
            >
              ...
              <div className="text-xs text-gray-500 dark:text-gray-400">{e.total_faqs} FAQ · {e.total_interacciones} interacciones</div>
            </button>
          ))}
        </div>
      )}
